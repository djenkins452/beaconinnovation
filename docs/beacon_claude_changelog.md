...
### Native Apple OTA install in the secure portal (device-aware, authorization-preserving)
- **Problem:** the product page's "Download Latest Version" linked straight to the authenticated IPA download, so Mobile Safari showed a raw `AIMSField.ipa` "Save…" instead of installing the app.
- **Root cause / architecture:** Apple OTA fetches the manifest and IPA with the iOS **system installer** (`itunesstored`), which carries **no Safari session cookie**. A `@login_required` (cookie) download therefore cannot back OTA, and a plain public manifest would break authorization. Redesigned around **short-lived signed tokens**: the product page (authenticated, authorized session) mints a `signing.dumps` token scoped to `(user, product)`; the manifest/IPA endpoints are **not** `@login_required` but require a valid, unexpired, correctly-scoped token **and** re-check `user_can_access` on every fetch.
- **New endpoints (products app):**
  - `GET /products/<slug>/manifest.plist?token=…` → dynamic Apple manifest (`plistlib`), `Content-Type: text/xml; charset=utf-8`, `Cache-Control: no-store`; embeds a fresh token in the `software-package` IPA URL (forced **https**).
  - `GET /products/<slug>/ota-download/?token=…` → streams the IPA as `application/octet-stream`, `no-store`.
  - Both return **403** (not a login redirect) on missing/invalid/expired/foreign/unauthorized tokens — the installer never hits a redirect.
- **Device-aware product page:** server-side UA detection (`iPhone`/`iPad`/`iPod`) + progressive JS for iPadOS-masquerading-as-macOS (`maxTouchPoints`). iOS → primary **Install AIMS** (`itms-services://?action=download-manifest&url=<url-encoded https manifest>`), secondary **Download IPA**. Desktop/Chrome/Edge → **Download IPA** primary, Install hidden.
- **Model:** added `Product.bundle_id` (CFBundleIdentifier, required for the manifest) + `is_ios_app` / `ota_capable` properties. `bootstrap_portal` now reads `bundle_id` from the IPA and **reconciles** it (and version/build) on every deploy even when the file is unchanged — backfilling products published before the field existed.
- **Deployment/settings:** added `SECURE_PROXY_SSL_HEADER=('HTTP_X_FORWARDED_PROTO','https')` so Django recognizes HTTPS behind Railway's TLS proxy (OTA requires https). `/healthz` now reports `aims.bundle_id` and `aims.ota_capable` for production verification.
- Files: `products/models.py`, `products/migrations/0004_product_bundle_id.py`, `products/views.py`, `products/urls.py`, `products/admin.py`, `products/management/commands/bootstrap_portal.py`, `products/templates/products/product_detail.html`, `products/tests.py`, `beaconinnovation/settings.py`, `website/views.py`.
- Verified on dev: iPhone UA → Install + itms link; Desktop UA → Install hidden, Download primary. Cookieless installer fetch: manifest 200 (text/xml, no-store, bundle-id/version/title correct, https IPA URL) → IPA 200 (octet-stream, 1,137,725 bytes, decodes to com.beaconinnovation.aims.field / 0.3.0 / build 3). Security: missing/tampered/foreign-signature/unauthorized-user/wrong-product tokens all → 403; anonymous product page → 302. Suite 490 passing.
- **Note (unchanged, separate):** the public `/static/downloads/` OTA page still serves the IPA unauthenticated (a distinct, pre-existing public distribution). The portal OTA is fully self-contained and does not depend on it — see Remaining concerns.
